...
# img_paths= ["img/test/00000080.jpg","img/test/00000090.jpg"]
#SIFT提取特征点
def extract_features(image_names):
    ...
    for image_name in image_names:
        ...
        # img = cv2.drawKeypoints(image, key_points, image,flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
        # cv2.imshow('img', img)
        # cv2.waitKey(0)
...
def match_features(query, train):
    ...
    knn_matches = bf.knnMatch(query, train, k=2)#使用K近邻算法匹配
    matches = []
    #Apply Lowe's SIFT matching ratio test(MRT)，值得一提的是，这里的匹配没有
    #标准形式，可以根据需求进行改动。
    for m, n in knn_matches:
        # if m.distance < config.MRT * n.distance:#判断匹配结果是否符合标准，MRT=0.7
        if m.distance < 0.7 * n.distance:#判断匹配结果是否符合标准，MRT=0.7
            matches.append(m)
...
def match_all_features(descriptor_for_all):
    ...
    for i in range(len(descriptor_for_all) - 1):
        matches = match_features(descriptor_for_all[i], descriptor_for_all[i + 1])#匹配当前图片和下一张图片
...
def find_transform(K, p1, p2):#K是相机解焦距参数矩阵
    focal_length = 0.5 * (K[0, 0] + K[1, 1])
    principle_point = (K[0, 2], K[1, 2])
    E, mask = cv2.findEssentialMat(p1, p2, focal_length, principle_point, cv2.RANSAC, 0.999, 1.0)#本函数用与计算本质矩阵E
    ...
    return R, T, mask# 本质矩阵分解为R和T，主要求解平移和旋转变换矩阵
...
def reconstruct(K, R1, T1, R2, T2, p1, p2):
    ...
    s = cv2.triangulatePoints(proj1, proj2, p1.T, p2.T)  # 三角法计算点云数据
    structure = []
...
def init_structure(K, key_points_for_all, colors_for_all, matches_for_all):
    ...
    for i, match in enumerate(matches):
        ...
        idx += 1
    return structure, correspond_struct_idx, colors, rotations, motions  # 通过调用init_structure获取需要的数据
...
def getRT(filePath):
    ...
    key_points_for_all, descriptor_for_all, colors_for_all = extract_features(img_paths)
    ...
    motions = translation_RT(motions)
    rotations = translation_RT(rotations)
    RT = np.hstack((motions, rotations))
    return RT
...
if __name__ == "__main__":
    ...
    # n=key_points_for_all.size
    # Rs=[]
    # Ts=[]
    # for i in range(n-1):
    #
    #     p1, p2 = get_matched_points(key_points_for_all[i], key_points_for_all[i+1], matches_for_all[i])#获取匹配的点
    #     R, T, mask = find_transform(K, p1, p2)
    #     Rs.append(R)
    #     Ts.append(T)
    # print(Rs,Ts)
